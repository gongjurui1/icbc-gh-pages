...
console.log('构造函数执行');
	...
	// ==================================
		...
		this.options.scrollY = false;
		this.options.momentum = false;
		var index = this.options.defaultIndex; //fwf 2016-09-27
		this.scroller = $('.ui-tab-content')[0];
		this.nav = $('.ui-tab-nav')[0];

		$(this.scroller.children[index]).addClass('current');
		$(this.nav.children[index]).addClass('current');
	...
	startX = -this.itemWidth*this.options.defaultIndex; //fwf 2016-09-27
	this.scrollTo(startX, this.options.startY); //fwf 2016-09-27
	this.enable();
